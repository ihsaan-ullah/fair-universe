...
class Model():
    ...
    def __init__(
        ...
        self.gamma_roi = None
        self.beta_roi = None
        self.theta_list = np.arange(self.best_theta, 1, 0.02)
    ...
    def predict(self, test_set):
        ...
        Y_hat_tests = self._predict_list(test_df, self.theta_list)
        ...
        i = 0
        for Y_hat_test,theta in zip(Y_hat_tests,self.theta_list):
                    
            nll = self.calculate_NLL(mu_scan, weights_test[Y_hat_test == 1],theta)
            if i == 0:
        ...
        if (mu_scan[np.where((hist_llr <= 1.0) & (hist_llr >= 0.0))].size == 0):
            ...
            mu_p16 = min(mu_scan[np.where((hist_llr <= 1.0) & (hist_llr >= 0.0))])
            mu_p84 = max(mu_scan[np.where((hist_llr <= 1.0) & (hist_llr >= 0.0))]) 
            mu_hat = mu_scan[np.argmin(hist_llr)]
    ...
    def _predict_list(self, X, theta_list=[0.9]):
        predictions = []
        theta_pre = min(theta_list) - 0.1
        for theta in theta_list:
            Y_predict = self._return_score(X)
            prediction = (np.where(Y_predict > theta_pre, 1, 0))
            predictions.append(np.where(prediction > theta, 1, 0))
            theta_pre = theta
    ...
    def mu_hat_calc(self):  
        ...
        Y_hat_mu_calc_sets = self._predict_list(self.mu_calc_set['data'], self.theta_list)
        ...
        gamma_roi_ = []
        beta_roi_ = []
        for i, Y_hat_mu_calc_set in enumerate(Y_hat_mu_calc_sets):
            Y_hat_mu_calc_set_signal = Y_hat_mu_calc_set[Y_mu_calc_set == 1]
            Y_hat_mu_calc_set_bkg = Y_hat_mu_calc_set[Y_mu_calc_set == 0]

            gamma_roi = (weights_mu_calc_set_signal[Y_hat_mu_calc_set_signal == 1]).sum()
            if gamma_roi == 0:
                gamma_roi = EPSILON

            gamma_roi_.append(gamma_roi)

            beta_roi_.append((weights_mu_calc_set_bkg[Y_hat_mu_calc_set_bkg == 1]).sum())
            self.gamma_roi = np.array(gamma_roi_)
            self.beta_roi = np.array(beta_roi_)
    ...
    def _validate(self):
        for valid_set in self.validation_sets:
            ...
            valid_set['predictions'] = self._predict_list(valid_set['data'], self.theta_list)
            valid_set['score'] = self._return_score(valid_set['data'])


    def _sigma_asimov_SR(self,mu,theta):
        index = np.where(self.theta_list == theta)
        return mu*self.gamma_roi[index] + self.beta_roi[index]

    def calculate_NLL( self,mu_scan, weight_data,theta):
        ...
        for i, mu in enumerate(mu_scan):
            hist_llr = (
                -2
                * sum_data_total_SR
                * np.log((self._sigma_asimov_SR(mu,theta) / self._sigma_asimov_SR(1.0,theta)))
            ) + (2 * (self._sigma_asimov_SR(mu,theta) - self._sigma_asimov_SR(1.0,theta)))
    ...
    def _compute_validation_result(self):
        ...
        for valid_set in self.validation_sets:
            ...
            Y_hat_valids = valid_set["predictions"]
            ...
            mu_scan = np.linspace(0, 5, 100)
            i = 0
            for Y_hat_valid,theta in zip(Y_hat_valids, self.theta_list):
                        
                nll = self.calculate_NLL(mu_scan, weights_valid[Y_hat_valid == 1],theta)
                if i == 0:
                    hist_llr = np.array(nll)
                else:
                    hist_llr += np.array(nll)
            ...
            if (mu_scan[np.where((hist_llr <= 1.0) & (hist_llr >= 0.0))].size == 0):
                ...
                p16_ = min(mu_scan[np.where((hist_llr <= 1.0) & (hist_llr >= 0.0))])
                p84_ = max(mu_scan[np.where((hist_llr <= 1.0) & (hist_llr >= 0.0))]) 
                mu_hat_ = mu_scan[np.argmin(hist_llr)]



            # get n_roi
            Y_hat_valid = self._predict(valid_set["data"], self.best_theta)
            n_roi = self.N_calc_2(weights_valid[Y_hat_valid == 1])
            # compute gamma_roi
            gamma_roi = self.gamma_roi.sum()
            beta_roi = self.beta_roi.sum()

            if gamma_roi == 0:
                gamma_roi = EPSILON

            mu_hats = ((n_roi - beta_roi)/gamma_roi)
            ...
            print(f"[*] --- p16: {np.round(p16, 4)} --- p84: {np.round(p84, 4)} --- mu_hat: {np.round(mu_hat, 4)}")
            print(f"[*] --- p16_: {np.round(p16_, 4)} --- p84_: {np.round(p84_, 4)} --- mu_hat_: {np.round(mu_hat_, 4)}")
