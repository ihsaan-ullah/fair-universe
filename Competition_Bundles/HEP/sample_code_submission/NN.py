...
class Model():
    ...
    def _train(self):
        ...
        self.train_set['predictions'] = self._predict(self.train_set['data'], 0.5)

    def _fit(self, X, y):
        data = torch.tensor(X.values, dtype=torch.float32)
        labels = torch.tensor(y, dtype=torch.float32)
        custom_dataset = CustomDataset(data, labels)
        data_loader = DataLoader(custom_dataset, batch_size=self.batch_size, shuffle=True)
    ...
    def _predict(self, X, theta):
        ...
        data_loader = DataLoader(custom_dataset, batch_size=self.batch_size, shuffle=False)
        self.NN.eval()
        predictions = []
        with torch.no_grad():
            for batch in data_loader:
