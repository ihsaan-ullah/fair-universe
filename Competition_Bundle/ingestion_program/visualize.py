...
def visualize_decision(ax, title, clf):
    ...
    response = clf.predict_proba(X_grid)
    response = response[:,1]
    response=response.reshape(xx0.shape)

    ax.set_title(title)
    plot_func = getattr(ax, plot_method)
    surface_ = plot_func(xx0, xx1, response,cmap=plt.cm.RdBu, alpha=0.5)
    ax.set_xlim([-8,8])
    ax.set_ylim([-8,8])
    ax.axhline(y=0, color='g', linestyle='--')
    ax.axvline(x=0, color='g', linestyle='--')
...
def visualize_decicion_boundary(name, settings, models, train_sets, test_sets):
    ...
    for index, model in enumerate(models):
        ...
        visualize_decision(ax, "Decision Boundry", model.clf)
        
      
        # train decision boundry
        ax = plt.subplot(1, 4, 3)
        visualize_decision(ax, "Train Data", model.clf)
        visualize_scatter(ax, train_sets[index])
       
        # test decision boundry
        ax = plt.subplot(1, 4, 4)
        visualize_decision(ax, "Test Data", model.clf)
