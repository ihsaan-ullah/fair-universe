    ...
    def validate(self):
        ...
        for valid_set in self.validation_sets:
            ...
            for theta in self.theta_candidates:
                ...
                Y_hat_valid = self._predict(valid_set['data'], theta)
                ...
                validation_sigma_squared.append(sigma_squared_mu_hat)

                # save predictions
                validation_predictions.append(Y_hat_valid)

            # save for this validation set
            # - best NROI
            # - theta
            # - predictions
            index_of_least_sigma_squared = np.argmin(validation_sigma_squared)
            validation_set_sigma_squared.append(validation_sigma_squared[index_of_least_sigma_squared])
            validation_set_thetas.append(self.theta_candidates[index_of_least_sigma_squared])
            validation_set_predictions.append(validation_predictions[index_of_least_sigma_squared])

        # get index of the least sigma squared
        best_index = np.argmin(validation_set_sigma_squared)

        # choose theta with least sigma squared
